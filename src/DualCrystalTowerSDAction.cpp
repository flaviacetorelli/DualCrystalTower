...
// it is faster but does not work as expected since cannot reproduce the expected rate
...
namespace dd4hep {
  ...
  namespace sim   {
     ...
     *
     * @{
     * \package DualCrystalTowerSDAction
     *
     * @}
     */

    /// Define collections created by this sensitivie action object
    template <> void Geant4SensitiveAction<DualCrystalTowerSD>::defineCollections()    {
      m_collectionID = declareReadoutFilteredCollection<CalVision::DualCrystalTowerSD::Hit>();
    }

    /// Method for generating hit(s) using the information of G4Step object.
    template <> bool Geant4SensitiveAction<DualCrystalTowerSD>::process(const G4Step* step,G4TouchableHistory* /*hist*/ ) {
	...
	//	if(SCEPRINT) std::cout<<"optical photon"<<std::endl;
         ...
         //std::cout<< rnd.rndm() <<std::endl;

        // does not work as expected 
        // keep 1 photon from 1000  
        //if(rnd.rndm()>supressRandom) {
	...
	//(track->GetCreatorProcess())->DumpInfo();
       ...
       */
       ...
       /*        
       if(SCEPRINT) {
       	   std::cout<<"NOT optical photon"<<std::endl; 
           std::cout<<"     hit energy is "<< contrib.deposit <<std::endl;
           std::cout<<"     hit acummulated  energy is "<< hit->energyDeposit  <<std::endl;
       }
      	*/
...
//--- Factory declaration
namespace dd4hep { namespace sim {
    typedef Geant4SensitiveAction<DualCrystalTowerSD> DualCrystalTowerSDAction;
  }}
DECLARE_GEANT4SENSITIVE(DualCrystalTowerSDAction)
